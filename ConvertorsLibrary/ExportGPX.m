...
- (instancetype)init
{
    ...
    self.lines = [NSMutableArray arrayWithCapacity:1000];

    return self;
}

#define LINE_S(__tag__, __value__) \
    [self.lines addObject:[NSString stringWithFormat:@"<%@>%@</%@>", __tag__, [MyTools HTMLEscape:__value__], __tag__]]
#define LINE_I(__tag__, __value__) \
    [self.lines addObject:[NSString stringWithFormat:@"<%@>%ld</%@>", __tag__, __value__, __tag__]]
...
- (void)header
{
    ...
    [self.lines addObject:@"<name>Geocube Export</name>"];
    [self.lines addObject:@"<desc>Geocache file generated by Geocube</desc>"];
    [self.lines addObject:@"<author>Geocube</author>"];
    [self.lines addObject:@"<email>geocube@mavetju.org</email>"];
...
*/
    ...
    [self.lines addObject:[NSString stringWithFormat:@"<wpt lat=\"%f\" lon=\"%f\">", wp.wpt_latitude, wp.wpt_longitude]];
    ...
    if (wp.gs_rating_difficulty != 0 && wp.gs_rating_terrain != 0) {
        [self.lines addObject:[NSString stringWithFormat:@"<groundspeak:cache id=\"%@\" archived=\"%@\" available=\"%@\" xmlns:groundspeak=\"http://www.groundspeak.com/cache/1/0/1\">", [NSNumber numberWithLongLong:wp._id], wp.gs_archived == YES ? @"true" : @"false", wp.gs_available == YES ? @"true" : @"false"]];
        ...
        if ([as count] == 0) {
            [self.lines addObject:@"<groundspeak:attributes />"];
        } else {
            [self.lines addObject:@"<groundspeak:attributes>"];
        ...
        if ([logs count] == 0) {
            [self.lines addObject:@"<groundspeak:logs />"];
        } else {
            [self.lines addObject:@"<groundspeak:logs>"];
            [logs enumerateObjectsUsingBlock:^(dbLog * _Nonnull log, NSUInteger idx, BOOL * _Nonnull stop) {
                [self.lines addObject:[NSString stringWithFormat:@"<groundspeak:log id=\"%ld\">", (long)log._id]];
                ...
                if (log.latitude != 0 && log.longitude != 0)
                    [self.lines addObject:[NSString stringWithFormat:@"<groundspeak:log_wpt lat=\"%f\" lon=\"%f\" />", log.latitude, log.longitude]];
                [self.lines addObject:@"</groundspeak:log>"];
            }];
            [self.lines addObject:@"</groundspeak:logs>"];
        }

        [self.lines addObject:@"<groundspeak:travelbugs />"];
...
- (void)writeToFile:(NSString *)filename
{
    ...
    [self.lines enumerateObjectsUsingBlock:^(NSString * _Nonnull l, NSUInteger idx, BOOL * _Nonnull stop) {
        [line appendString:l];
        [line appendString:@"\n"];
    }];
